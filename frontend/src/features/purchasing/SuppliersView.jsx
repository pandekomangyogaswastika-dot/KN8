...
import { Truck, Plus, Search, Pencil, Power, Building2, X } from "lucide-react";
...
const EMPTY_FORM = {
  ...
  address: "", city: "", goods_type: "", payment_term_code: "", entity_id: "", notes: "",
};
...
export default function SuppliersView({ currentUser, selectedEntity }) {
  ...
  async function handleSubmit() {
    ...
    try {
      if (editId) {
        await axios.patch(`${API}/suppliers/${editId}`, { data: form });
  ...
  const entityName = (id) => entities.find((e) => e.id === id)?.short_name || entities.find((e) => e.id === id)?.legal_name || "—";
  ...
  return (
    <div data-testid="suppliers-view">
      ...
      {error && (
        <div className="notice-bar danger" data-testid="supplier-error">
      ...
      <div className="section-card">
        <div className="overflow-hidden">
          <div className="grid grid-cols-[90px_1.4fr_1fr_120px_110px_80px] px-3 py-1.5 bg-[#FAFBFC] text-[10px] font-bold uppercase text-[#6B6B73] border-b border-[#EFF0F2]">
            <span>Kode</span><span>Nama / Jenis</span><span>NPWP / Kontak</span><span>Kota / Entitas</span><span>Status</span><span className="text-right">Aksi</span>
          </div>
          {loading ? (
            ...
            <div className="divide-y divide-[#EFF0F2] max-h-[600px] overflow-y-auto">
              {filtered.map((s) => (
                <div key={s.id} data-testid={`supplier-row-${s.id}`}
                  className="grid grid-cols-[90px_1.4fr_1fr_120px_110px_80px] items-center px-3 py-2.5 hover:bg-[#FAFBFC]">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10.5px] text-[#6B6B73] truncate flex items-center gap-1"><Building2 size={10} />{entityName(s.entity_id)}</p>
                  </div>
