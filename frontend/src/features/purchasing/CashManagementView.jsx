...
export default function CashManagementView({ currentUser, selectedEntity }) {
  ...
  async function handleVoid(t) {
    if (!window.confirm(`Void transaksi ${t.number}? Saldo akan disesuaikan.`)) return;
    try {
  ...
  return (
    <div data-testid="cash-management-view">
      ...
      {error && <div className="notice-bar danger" data-testid="cash-error"><span>{error}</span><button onClick={() => setError("")}><X size={13} /></button></div>}
