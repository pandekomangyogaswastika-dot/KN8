...
export default function PurchaseReturns({ currentUser, selectedEntity }) {
  ...
  async function handleSubmit() {
    const items = form.items.filter((it) => it.product_id && Number(it.quantity) > 0)
      .map((it) => ({ product_id: it.product_id, quantity: Number(it.quantity), unit: "meter", price: Number(it.price || 0), reason: it.reason, condition: it.condition }));
  ...
  return (
    <div data-testid="purchase-returns-view">
      ...
      {error && <div className="notice-bar danger" data-testid="pret-error"><span>{error}</span><button onClick={() => setError("")}><X size={13} /></button></div>}
        ...
        <div className="overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="divide-y divide-[#EFF0F2] max-h-[560px] overflow-y-auto">
              {filtered.map((r) => (
                <div key={r.id} data-testid={`return-row-${r.id}`} className="grid grid-cols-[100px_1.3fr_110px_120px_110px_120px] items-center px-3 py-2.5 hover:bg-[#FAFBFC]">
                  ...
                  <div className="flex items-center justify-end gap-1.5">
                    ...
                    {r.status === "pending_approval" && canApprove && (
                      <>
                        ...
                        <button data-testid={`return-reject-${r.id}`} onClick={() => { const n = window.prompt("Alasan tolak?", ""); if (n !== null) act(r.id, "reject", { notes: n }); }} className="danger-button !px-2 !py-1 text-[11px]"><XCircle size={11} /></button>
                      </>
                    )}
