...
import {
  AlertCircle, ArrowLeft, Check, CheckCircle2, Clock, Loader2,
...
export default function SpecialOrderDetail({
  ...
  async function handleApprove() {
    ...
    try {
      const res = await axios.post(
        `${API}/api/special-orders/${order.id}/approve`,
        { notes: "" },
  ...
  async function handleReject() {
    ...
    try {
      const res = await axios.post(
        `${API}/api/special-orders/${order.id}/reject`,
        { reason: rejectReason },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      onUpdate(res.data);
  ...
  async function handleStatusTransition(newStatus) {
    ...
    try {
      const res = await axios.post(
        `${API}/api/special-orders/${order.id}/status`,
