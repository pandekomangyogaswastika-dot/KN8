...
export default function ReturnDetail({
  ...
  async function uploadFile(file) {
    ...
    try {
      ...
      fd.append("file", file);
      await axios.post(
        `${API}/api/sales-returns/${ret.id}/attachments`,
        fd,
        { headers: { Authorization: `Bearer ${token}`, "Content-Type": "multipart/form-data" } }
      );
      const updated = await axios.get(`${API}/api/sales-returns/${ret.id}`, {
            ...
            <div className="attachments-grid">
              {attachments.map(att => (
                <div key={att.id} className="attachment-chip" data-testid={`attachment-${att.id}`}>
                  <Paperclip size={11} />
                  <a href={`${API}/api/sales-returns/${ret.id}/attachments/${att.id}/download`}
