...
export default function ApprovalRulesSettings({ currentUser }) {
  ...
  async function loadRules() {
    setLoading(true);
    try {
      const res = await axios.get(`${API}/api/approval-rules`, {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editingRule) {
        // Update
        await axios.patch(
          `${API}/api/approval-rules/${editingRule.id}`,
  ...
  async function handleDelete(rule) {
    ...
    try {
      await axios.delete(`${API}/api/approval-rules/${rule.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  async function toggleActive(rule) {
    try {
      await axios.patch(
        `${API}/api/approval-rules/${rule.id}`,
        { is_active: !rule.is_active },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      loadRules();
